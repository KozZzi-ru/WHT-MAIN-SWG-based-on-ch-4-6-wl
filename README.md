...
- **Раздельная скорость вертикального и горизонтального скролла** (бранч `separate-scroll-speed-v-h`)
...
### 10.02.2026 - Оптимизация для медленных BT-адаптеров (Baseus BA04)
- Создан бранч `fix-bt-baseus-adapter`
- Проблема: периодические зависания только на ПК с Baseus BA04, на ноутбуках работает стабильно
- Причина: дешёвый USB BT-адаптер не успевает обрабатывать поток HID-событий от трекбола
- **Изменения в `charybdis.conf`:**
  - Увеличен BT connection interval: 9ms → 15ms (снижена частота пакетов)
  - Увеличена BT latency: 16 → 30 (больше времени на обработку)
  - Добавлен `CONFIG_ZMK_USB_HID_REPORT_RATE=125` (лимит 125 Hz для HID reports)
- **Изменения в `charybdis_right.overlay`:**
  - Добавлен `poll-interval-ms = <8>` для трекбола (polling rate ~125 Hz вместо ~1000 Hz)
  - Снижена нагрузка на BT-стек за счёт редкого опроса сенсора
...
### 02.02.2026 - Актуализация README: исправлены CPI и масштабы всех режимов
- Исправлено значение CPI: 1600 → 1000 (соответствует реальной конфигурации)
- Снайперский режим: масштаб исправлен с 1:5 на 1:3
- Режим мыши: масштаб исправлен с 2:3 на 1:1 (нативная скорость)
- Медленный скролл: масштаб исправлен с 1:240 на 1:120, горизонтальный с 1:4 на 1:3
- Быстрый скролл: масштаб исправлен с 1:60 на 1:40, горизонтальный 1:4 (верно)
- Обновлена документация по настройке чувствительности трекбола

### 02.02.2026 - Раздельная скорость вертикального и горизонтального скролла
- Создан новый бранч `separate-scroll-speed-v-h`
- Добавлены кастомные скейлеры `zip_wheel_scaler` (вертикальный) и `zip_hwheel_scaler` (горизонтальный)
- На базовом слое (QWERTY): вертикальный скролл - обычная скорость (1:1), горизонтальный - в 4 раза медленнее (1:4), общий масштаб 1:40
- На слое NUM_AND_FUN: вертикальный скролл - обычная скорость (1:1), горизонтальный - в 3 раза медленнее (1:3), общий масштаб 1:120
- Использованы коды событий INPUT_REL_WHEEL и INPUT_REL_HWHEEL для независимого управления осями
- Каждая ось скролла теперь имеет собственный масштабирующий процессор
...
- Режим мыши (AUTO_MOUSE, слой 2): масштаб 1:1 - нативная скорость
