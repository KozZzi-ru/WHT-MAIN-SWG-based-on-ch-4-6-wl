...
- **`main`** — основная стабильная ветка для Charybdis 4x6 wireless MK1 WHITE на ZMK v0.3. Используй эту ветку для повседневной прошивки.
- **`READY_true-dual-scroll-1600cpi_zmk4_Badjeff`** — экспериментальная ветка с ZMK main (Zephyr 4.1), драйвером от badjeff и продвинутыми input processors. Для тестирования новых возможностей.
- **`badjeff-driver`** — экспериментальная ветка с альтернативным драйвером трекбола от badjeff.
- **`zmk-main-update`** — рабочая ветка для обновления ZMK на более новые версии и тестов.
